...
/**
 * Les six langues sont générées à la construction.
 *
 * C'est tout l'intérêt d'avoir mis la langue dans l'adresse : les pages
 * publiques restent statiques, une version par langue, et un moteur de
 * recherche trouve les six. Une liste vide ne ferait échouer aucune
 * construction — elle rendrait simplement 404 en production.
 */
export function generateStaticParams() {
  return toutesLesLocales();
}
...
export default async function RootLayout({
  ...
  // Un premier segment qui n'est pas une langue n'est pas une page : sans ce
  // contrôle, `/nimportequoi` rendrait la mise en page entière autour d'un
  // 404, et `<html lang>` annoncerait une langue qui n'existe pas.
  ...
  return (
    <html lang={locale} className={`h-full ${titrage.variable} ${barlow.variable} ${plexMono.variable}`}>
      <head>
        {/*
